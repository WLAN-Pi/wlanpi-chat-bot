...
class RemoteConfig(object):
    ...
    def __init__(
        ...
        timestamp_file="/tmp/config_timestamp.dat",
        username="",
        password="",
    ):
    ...
    def read_remote_cfg(self):
        ...
        class_logger.debug(
            "Trying to pull config file from : {}".format(self.cfg_file_url)
        )
        try:
            # warnings.simplefilter('ignore',InsecureRequestWarning)
            response = requests.get(
                self.cfg_file_url,
                auth=(self.cfg_username, self.cfg_password),
                timeout=5,
            )
    ...
    def check_cfg_timestamp(self):
        ...
        class_logger.debug(
            "Checking time diff, time now: {}, last read time: {}, interval: {}, current diff: {}".format(
                time_now, last_read_time, self.cfg_refresh_interval, current_diff
            )
        )
